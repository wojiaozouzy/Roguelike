...
impl Vector2Int {
    ...
    pub fn manhattan(&self, ohter: Vector2Int) -> i32 {
        (self.x - ohter.x).abs() + (self.y - ohter.y).abs()
    }
}
impl Add for Vector2Int {
    type Output = Self;
    fn add(self, ohter: Self) -> Self {
        return Vector2Int::new(self.x + ohter.x, self.y + ohter.y);
    }
}
